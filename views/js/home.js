...
// Javascript code to prevent empty mail-submission. Usage of event-propagation.
// When both eventListeners are set to false, we will first see the Button alert since it looks to the bubbling fase 
// and therefore starts at the most nested item; button. After that it sees the div which nests the button. 

// When both eventListeners are set to true, it looks to the capturing fase (going down in the hierarchy). Since 
// div nests button, we first see the Div alert and after that the Button alert. 
var button = document.getElementById('button_1');
var buttondiv = document.getElementById('buttondiv');
button.addEventListener('click',buttonclickempty, true)
buttondiv.addEventListener('click',buttondivclickempty, true)


function buttonclickempty(){
    input = document.getElementById('newsletterform')[0].value;
    if(input == ''){
        alert('Button alert: Your submission is empty. Please, enter a valid e-mailadress.')
    }
}

function buttondivclickempty(){
    input = document.getElementById('newsletterform')[0].value;
    if(input == ''){
        alert('Div alert: Your submission is empty. Please, enter a valid e-mailadress.')
    }
}

// Make menu's and fill it with options
